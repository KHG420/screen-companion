...
这是可继续开发和测试的 0.4.3 测试版。不含账号、远程控制或录屏存储。共享时支持采集 Android 10+ 允许捕获的媒体/游戏声音，并与麦克风混合发送。没有真机验证的高清/低延迟承诺。屏幕视频、麦克风及共享媒体声音经 WebRTC 传输，信令 HTTP 长轮询只交换 SDP、ICE 和共享状态，不转发媒体。
...
0.4.3 中，“自动平衡”“帧率优先”和高于 30 FPS 的目标使用动态画面编码提示；30 FPS 以内的“清晰优先”保留屏幕文字编码路径。自动平衡可在负载过高时调节分辨率和/或帧率，不固定降低画质。仅调整码率时保留采集配置。统计新增按周期计算的编码/帧、解码/帧、发送排队/包和接收缓冲/帧耗时；这些分段测量不能当作端到端延迟。

新增 **4K · 60 帧** 预设：3840×2160、60 FPS、40 Mbps 上限，使用分辨率优先。高帧率的动态编码提示与资源不足时的降级策略独立，因此不会为了达到 60 FPS 而主动选择降分辨率。共享源本身必须有 4K，不通过放大冒充。浏览器在系统报告具备 4K60 H.264 平滑编码/解码能力时，优先使用双方支持的高效 H.264 档次，同等能力时保留浏览器原有顺序，并保留其他编码回退；安卓只在 WebRTC 硬件工厂支持的设备上开放 High Profile。预设不是设备达标保证，需同时看实际发送、接收分辨率及 FPS。
